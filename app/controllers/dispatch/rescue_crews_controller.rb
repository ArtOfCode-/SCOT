class Dispatch::RescueCrewsController < ApplicationController
  ...
  def index
    respond_to do |format|
      format.html do
        @crews = Dispatch::RescueCrew.all.includes(:request_status).paginate(page: params[:page], per_page: 50)
      end
      format.json do
  ...
  def show
    @crew = Dispatch::RescueCrew.includes(:status, :rescue_requests, rescue_requests: [:request_status, :request_priority]).where(id: params[:id]).first
  end
